...
export default function() {
  ...
  responses.register = http.post(`${BASE_URL}/api/auth/register`, JSON.stringify(registerPayload), {
  ...
  responses.login = http.post(`${BASE_URL}/api/auth/login`, JSON.stringify(loginPayload), {
    headers: { 'Content-Type': 'application/json' },
  });
  ...
  if (authToken) {
    ...
    responses.createContact = http.post(`${BASE_URL}/api/contacts`, JSON.stringify(contactPayload), {
      headers: { 
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${authToken}`
      },
    ...
    if (contactId) {
      ...
      check(responses.getContact, {
        ...
        'get contact has correct email': (r) => {
          try {
            const body = JSON.parse(r.body);
            return body.data && body.data.contact && body.data.contact.email === contactPayload.email;
          } catch (e) {
            return false;
          }
        },
      ...
      responses.updateContact = http.put(`${BASE_URL}/api/contacts/${contactId}`, JSON.stringify(updatePayload), {
        headers: { 
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${authToken}`
        },
      ...
      check(responses.updateContact, {
        ...
        'update contact reflects changes': (r) => {
          try {
            const body = JSON.parse(r.body);
            return body.data && body.data.contact && body.data.contact.firstName === 'Jane';
          } catch (e) {
            return false;
          }
        },
      ...
      responses.deleteContact = http.del(`${BASE_URL}/api/contacts/${contactId}`, null, {
        headers: { 
          'Authorization': `Bearer ${authToken}`
        },
      });
