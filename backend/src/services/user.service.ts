...
import { UserRepository, UserFilters, UserSearchOptions } from '../repositories/user.repository';
import { 
  NotFoundError,
  ConflictError,
  ForbiddenError,
...
export class UserService {
  ...
  async updateUser(
    ...
    try {
      ...
      if (!canUpdate.allowed) {
        throw new ForbiddenError(canUpdate.reason || 'Insufficient permissions');
      }
      ...
      if (updates.isVerified !== undefined && requestingUser.role !== UserRole.ADMIN) {
  ...
  async deactivateUser(
    ...
    try {
      ...
      if (user.role === UserRole.ADMIN) {
        const adminCount = await this.userRepository.count({ role: UserRole.ADMIN, is_active: true });
        if (adminCount <= 1) {
  ...
  async getUsersByRole(
    ...
    try {
      // Check permissions
      if (requestingUser.role === UserRole.USER) {
        throw new ForbiddenError('Insufficient permissions to filter users by role');
      }
  ...
  private canUpdateUser(
    ...
    if (requestingUser.role === UserRole.MANAGER && targetUser.role === UserRole.USER) {
      return { allowed: true };
    }
    
    return { allowed: false, reason: 'Insufficient permissions to update this user' };
  }
  ...
  async bulkUpdateUsers(
    ...
    try {
      ...
      await Promise.all(
        updates.map(update => this.clearUserCache(update.id))
      );
