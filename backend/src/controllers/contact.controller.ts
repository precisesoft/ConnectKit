...
export class ContactController {
        ...
        filters: {
          ...
          hasEmail:
            req.query.hasEmail === 'true'
              ? true
              : req.query.hasEmail === 'false'
                ? false
                : undefined,
          hasPhone:
            req.query.hasPhone === 'true'
              ? true
              : req.query.hasPhone === 'false'
                ? false
                : undefined,
   ...
   * Archive contact
   */
  archiveContact = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      const currentUser = (req as any).user;
      const { id } = req.params;

      const contact = await this.contactService.archiveContact(
        id,
        currentUser.id
      );
   ...
   */
  restoreContact = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      const currentUser = (req as any).user;
      const { id } = req.params;

      const contact = await this.contactService.restoreContact(
        id,
        currentUser.id
      );
  ...
  toggleFavorite = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      ...
      const contact = await this.contactService.toggleFavorite(
        id,
        currentUser.id
      );
  ...
  removeTag = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      ...
      const contact = await this.contactService.removeTag(
        id,
        currentUser.id,
        tag
      );
  ...
  getFavorites = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      ...
      const result = await this.contactService.getFavorites(
        currentUser.id,
        options
      );
   ...
   */
  getContactsByStatus = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      const currentUser = (req as any).user;
   ...
   */
  bulkUpdateContacts = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      const currentUser = (req as any).user;
      ...
      if (
        !request.contactIds ||
        !Array.isArray(request.contactIds) ||
      ...
      const results = await this.contactService.bulkUpdateContacts(
        currentUser.id,
        request
      );
   ...
   */
  findDuplicates = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      const currentUser = (req as any).user;
      const duplicates = await this.contactService.findDuplicates(
        currentUser.id
      );
  ...
  mergeDuplicates = asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      ...
      if (
        !duplicateContactIds ||
        !Array.isArray(duplicateContactIds) ||
